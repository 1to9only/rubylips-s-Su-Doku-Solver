...
public class Composer extends Thread {
    ...
    public Composer( GridContainer gridContainer ,
        ...
        if( xmlFormat && this.output instanceof PrintWriter ){
            this.output.println("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>");
            this.output.println("<sudoku-book>");
            this.output.println("<note>Puzzles generated by " + getClass() + " on " + DateFormat.getDateTimeInstance().format( new Date() ) + ".</note>");
            this.output.println("<user>0</user>");
            this.output.println("<last>000000000000</last>");
            this.output.println("<checked>000000000000</checked>");
            this.output.println("<xtra>0</xtra>");            
            this.output.print("<puzzle-type>");
            this.output.print( cellsInRow == 9 && boxesAcross == 3 ? '0' : '1' );
            this.output.println("</puzzle-type>");
            this.output.println("<cells-in-row>" + cellsInRow + "</cells-in-row>");
            this.output.println("<boxes-across>" + boxesAcross + "</boxes-across>");
            this.output.println("<boxes-down>" + cellsInRow / boxesAcross + "</boxes-down>");
            
            i = 0 ;
            while( i < featuredGrades.length ){
                this.output.println("<featuredGrade>" + featuredGrades[i] + "</featuredGrade>");
                ++ i ;
            }
        }
    }
    ...
    public synchronized void addSolution( int solverIndex ){
        ...
        if( ! puzzles.contains( puzzle ) ){
            ...
            if( output instanceof PrintWriter ){
                if( xmlFormat ){
                    puzzle.toXML( output , 1 + nSolns , featuredGrades[category] );
                } else {
